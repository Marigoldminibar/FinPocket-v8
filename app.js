...
(() => {
  ...
  // Bildirimler için yalnızca minimal Service Worker kaydı.
  // Fetch/cache/respondWith kullanılmaz.
  if ('serviceWorker' in navigator && (location.protocol === 'https:' || location.hostname === 'localhost')) {
    navigator.serviceWorker.register('./service-worker.js', { updateViaCache: 'none' })
      .then(reg => reg.update().catch(() => {}))
      .catch(err => console.warn('Bildirim Service Worker kaydı başarısız:', err));
  }
  ...
  // QR cihaz eşleştirme kaldırıldı. FinPocket her cihazda bağımsız çalışır.
...
backup: $('backupBtn'), report: $('reportBtn'), restore: $('restoreBtn'), restoreFile: $('restoreFile'), reset: $('resetBtn'),    home: $('homeBtn'), cards: $('cardsBtn'), money: $('moneyBtn'), settings: $('settingsBtn')
  };
...
function restore(file) {
  ...
  function updateNotificationButton() {
    ...
    // Uygulama içindeki durum, sistem izin durumundan bağımsız tutulur.
    // Böylece kullanıcı Aç dediğinde sekmede gerçekten Açık görünür.
    const enabled = Notifications.isEnabled();
    btn.textContent = enabled ? '🔔 Bildirimler Açık' : '🔕 Bildirimler Kapalı';
    btn.setAttribute('aria-pressed', enabled ? 'true' : 'false');
    btn.title = enabled
      ? 'Bildirimler açık. Kapatmak için dokunun.'
      : 'Bildirimleri açmak için dokunun.';
  }
  ...
  document.addEventListener('click', e => {
    ...
    if (id === 'notificationBtn') {
      ...
      return Notifications.requestPermission().then(async result => {
        if (result === 'granted') {
          Notifications.setEnabled(true);
          updateNotificationButton();
          const sent = await Notifications.send('FinPocket', 'Bildirimler başarıyla açıldı.');
          if (location.protocol === 'file:') {
            alert('🔔 Bildirimler açıldı.\n\nBu dosya bilgisayarda file:// olarak açıldığı için gerçek PWA bildirim testi yapılamaz. GitHub Pages (HTTPS) üzerinde test etmeliyiz.');
          } else if (sent) {
            alert('🔔 Bildirimler açıldı. Test bildirimi gönderildi.');
          } else {
            alert('🔔 Bildirimler açık. Ancak test bildirimi gönderilemedi.');
          }
          await Notifications.run();
        } else if (result === 'denied') {
          Notifications.setEnabled(false);
          updateNotificationButton();
          alert('Bildirim izni engellendi. iPhone/Android ayarlarından FinPocket bildirimlerine izin verin.');
        } else {
          Notifications.setEnabled(false);
          updateNotificationButton();
