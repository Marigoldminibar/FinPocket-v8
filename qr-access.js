// FinPocket - QR şifreli yeni cihaz erişimi
// QR sistemi finans verisini paylaşmaz. Her cihaz kendi localStorage verisini kullanır.
...
async function getQrFirebase() {
  if (!qrFirebasePromise) {
    qrFirebasePromise = (async () => {
      const [{ initializeApp, getApps, getApp }, { getAuth, signInAnonymously }, { getDatabase, ref, get, set }] = await Promise.all([
...
async function createQrAccess() {
  ...
  createBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      const appBaseUrl = location.protocol === 'file:'
        ? 'https://marigoldminibar.github.io/FinPocket-v8/'
        : `${location.origin}${location.pathname}`;
      const qrUrl = `${appBaseUrl}?qr=${encodeURIComponent(token)}`;
      passwordOverlay.remove();

      const qrOverlay = showQrOverlay({
        title: 'QR Hazır',
        body: 'Bu QR yeni cihazda boş bir FinPocket açar. Finans verilerin bu QR ile paylaşılmaz.',
        content: `
      ...
      // QR görselini doğrudan HTTPS QR servisi ile oluştur.
      // Böylece file://, Safari ve Chrome ortamlarında canvas/kütüphane sorunu yaşamayız.
...
async function requireQrAccess(token) {
  let accessGranted = sessionStorage.getItem(`fp_qr_access_${token}`) === '1';
  ...
  const verify = async () => {
    ...
    try {
      ...
      const hash = await derivePasswordHash(password, data.salt);
...
// QR sistemi uygulamanın diğer bölümlerinden bağımsızdır.
window.FinPocketQR = {
  create: createQrAccess,
  require: requireQrAccess
};

// QR linki ile gelen yeni cihazı önce şifre kapısında tut.
(async () => {
  try {
...
// Ayarlar içindeki QR butonunu mevcut uygulama koduna dokunmadan ele al.
